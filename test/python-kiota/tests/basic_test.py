...
def registry_server(request):
    ...
    subprocess.call(f"{DOCKER} compose -f docker-compose.yaml build", shell=True, cwd=root_folder)
    p = subprocess.Popen(f"{DOCKER} compose -f docker-compose.yaml up", shell=True, cwd=root_folder)
...
def cleanup():
    ...
    subprocess.call(f"{DOCKER} compose -f docker-compose.yaml down", shell=True, cwd=root_folder)
...
async def test_model_version_create_and_retrieve():
    ...
    payload.registered_model_id = create_registered_model.id
...
async def test_model_artifact_create_and_retrieve():
    ...
    mv.registered_model_id = create_registered_model.id
